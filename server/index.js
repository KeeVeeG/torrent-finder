...
app.get('/', (req, res) => {
  ...
  axios
    .get(encodeURI(ip + '/search/0/0/010/2/' + encodeURI(req.query.name) + '/'))
    .then(response => {
      ...
      list = list.filter(
        e =>
          e.querySelectorAll('td')[1].querySelectorAll('a')[0] !== undefined &&
          e.querySelectorAll('td')[1].querySelectorAll('a')[1] !== undefined
      )
      list = list.map(e => {
        ...
        return {
          ...
          peers: GetPeers(
            e.querySelectorAll('td')[4].querySelector('.green').innerHTML
          ),
        }
      })
...
app.get('/magnet', (req, res) => {
  ...
  axios.get(encodeURI(ip + req.query.href)).then(response => {
    ...
    res.send(
      doc
        .querySelector('#download_page_buttons')
        .querySelectorAll('a')[1]
        .getAttribute('href')
    )
  })
...
app.get('/description', (req, res) => {
  ...
  axios.get(encodeURI(ip + req.query.href)).then(response => {
    ...
    const data = response.data
    const doc = new JSDOM(data).window.document
    ;[].forEach.call(doc.querySelectorAll('img'), e =>
      e.setAttribute('onError', "this.style.display='none'")
    )
    res.send(
      doc
        .querySelectorAll('#details>tbody>tr')[0]
        .innerHTML.replace(/\<a href=".*?\">/g, '')
    )
  })
})
